 ...
 * the SUFFIX: a t-shirt size (`text-xs`) or an explicitly-typed arbitrary value
 * (`text-[length:...]`) is a size, and ANYTHING ELSE is a colour. It has no
 * access to the compiled theme, so a custom scale is invisible to it.
 *
 ...
 *
 * This is also why the arbitrary values these aliases replaced were all written
 * as `text-[length:var(--app-text-base)]` rather than `text-[var(...)]`: the
 * `length:` hint was doing this same job inline. Naming the scale moved that
 * job here.
 */
